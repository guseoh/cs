...
DNS는 domain과 address를 연결하지만 port를 열거나 service health를 보장하지 않는다. Public address가 바뀌었는데 DNS TTL 동안 이전 값이 cache되면 일부 client는 과거 instance로 갈 수 있다. Elastic IP를 사용하면 address를 고정할 수 있지만 public IPv4 비용과 release 위험이 생긴다.
