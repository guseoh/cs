...
이 방식이 사용되는 이유를 이해하려면 계정 잠금 정책을 생각해 볼 수 있다. 서비스가 “한 계정에서 로그인 실패 5회가 발생하면 잠금” 같은 정책을 사용한다고 가정해 보자. 하나의 계정에 비밀번호를 수백 개 넣으면 금방 차단될 수 있다. 하지만 계정을 계속 바꾸면서 각 계정에는 한두 번만 요청하면 단순한 계정별 실패 횟수 제한을 우회하기 쉬워진다.
...
이런 계산 비용은 비밀번호 보호에는 좋은 특성이지만, 서버 입장에서는 다른 의미도 생긴다.
...
여기서 Rate Limiting을 “보안 공격만 막는 기능”으로 좁게 생각하면 안 된다. Rate Limiter는 다음과 같은 목적으로 사용할 수 있다.
...
따라서 IP 기반 Rate Limiting의 한계를 이해하지 않고 “IP당 10회면 Credential Stuffing이 해결된다”고 생각하면 안 된다.
