...
최종 코드 HEAD 기준 검증은 두 번 닫았다.

첫 번째는 Draft 상태에서 correction이 끝난 직후의 검증이다.
...
그 뒤 PR을 Ready로 전환하면서 같은 HEAD에서 workflow가 다시 실행됐다. 코드 변경은 없었지만 merge 직전 상태까지 다시 확인했다.

```text
Repository Validation #1751
├─ Commit and PR conventions   PASS
├─ Classify validation changes PASS
├─ Harness validation          PASS
├─ Backend and MySQL           PASS
...
PR Metadata Validation #503    PASS
```

즉 최종 HEAD `803cb6690d9de54b8412fca81dec27d36785ea29`는 Draft correction 완료 시점과 Ready 전환 이후 시점 모두에서 Repository Validation과 PR Metadata Validation을 통과했다.
...
병합 직후 `main`의 최신 commit도 `df74023e96a3afe37219ecd50d48adac8b7b3c60`과 일치하는 것을 다시 확인했다.
